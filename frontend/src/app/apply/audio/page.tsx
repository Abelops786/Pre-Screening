...
function AudioContent() {
  ...
  useEffect(() => {
    if (!candidateId) router.replace('/apply');
  }, [candidateId, router]);
      ...
      <div className="w-full max-w-xl">
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-2 bg-white/10 text-white rounded-full px-4 py-1.5 text-sm mb-4">
            <Mic size={16} />
            Step 3 of 3 – Audio Interview
          </div>
          <h1 className="text-2xl font-bold text-white mb-1">Voice Screening</h1>
          <p className="text-brand-100 text-sm">Your spoken response will be analyzed for language fluency</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-6 space-y-5">
          ...
          {phase !== 'done' && (
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Screening Prompt</p>
          ...
          {phase === 'ready' && (
            <div className="text-center space-y-4">
              ...
              >
                <div className="w-20 h-20 rounded-full bg-brand-600 hover:bg-brand-700 flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all">
          ...
          {phase === 'recorded' && audioUrl && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  onClick={reset}
                  className="flex-1 border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium rounded-xl py-2.5 flex items-center justify-center gap-2 transition-colors text-sm"
                >
                  <RefreshCw size={16} />
                  Re-record
                </button>
                <button
                  onClick={submit}
                  className="flex-1 bg-brand-600 hover:bg-brand-700 text-white font-semibold rounded-xl py-2.5 flex items-center justify-center gap-2 transition-colors text-sm"
                >
                  <Upload size={16} />
                  Submit Recording
                </button>
          ...
          {phase === 'uploading' && (
            <div className="flex flex-col items-center gap-4 py-6">
              ...
              <p className="text-gray-600 text-sm text-center">Uploading and analyzing your audio…<br />This may take up to 60 seconds.</p>
            </div>
          )}
